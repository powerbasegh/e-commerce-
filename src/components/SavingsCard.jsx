...
export default function SavingsCard() {
  return (
    <aside
      ...
      className="hidden w-[250px] shrink-0 flex-col justify-between rounded-xl border border-pb-gray-border bg-white p-4 shadow-card xl:flex"
    >
      <div>
        <p className="text-[14px] font-extrabold text-pb-gray-text">Big Savings Today!</p>
          ...
          {FEATURES.map((f) => (
            <li key={f.title} className="flex items-start gap-3">
              ...
              <span className="min-w-0">
                <span className="block text-[10px] font-bold text-pb-gray-text">{f.title}</span>
                <span className="block text-[9px] leading-snug text-pb-gray-muted">{f.text}</span>
              </span>
            </li>
          ))}
        </ul>
      ...
      <a
        href="/flash-deals"
        className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-pb-green py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-pb-green-dark"
      >
